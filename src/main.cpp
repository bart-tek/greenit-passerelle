...
int GiveID(int serailnumber){
  ...
  {
    if (AdresseNodeID[i] == 0 || AdresseNodeID[i] == serailnumber) {
...
int sendtheIDToTheNode(int serialnumber){
  ...
  int idToGive = GiveID(serialnumber);

  sprintf(buf, "%i;%i;%i;", idToGive, MYNETID, serialnumber);
  len = strlen(buf);

  return sendLORA(MYNETID, 0x01, 0x00, 0x01, 0x01, buf, len);
}
...
void loop() {
  ...
  if (receivLoRa()==1) {
    ...
    if ((myMsg.idx == MYNETID)&&(myMsg.dst==0x01)&&(myMsg.src==0x00)&&(myMsg.sdx=0x01)) {
    ...
    // Q5 définissez le filtrage
    if ((myMsg.idx == MYNETID)&&(myMsg.dst==0x01)&&(myMsg.sdx=0x02)&&(myMsg.cmd==0x00)) {
